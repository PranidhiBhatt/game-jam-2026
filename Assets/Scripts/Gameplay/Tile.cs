...
/// <summary>
/// Represents an individual grid tile in Dangerous Arena.
/// Handles visual representation, dynamic type changes, and provides
/// decoupled event hooks for player interaction and level management.
/// </summary>
[SelectionBase]
...
public class Tile : MonoBehaviour
{
    ...
    private void Start()
    {
        // Ensure visual state is current after initialization
        UpdateVisual();
    }
    ...
    {
        // Live update inspector adjustments in the Scene view
    ...
    /// <summary>
    /// Changes the tile's gameplay type dynamically, immediately updates its visuals,
    /// and notifies subscribed gameplay listeners.
    /// </summary>
    ...
    /// Safely updates the Renderer material or color without leaking materials in the Editor.
    /// </summary>
    public void UpdateVisual()
    {
        ...
        // 1. If an explicit material is assigned for this TileType, use it
        ...
        if (customMaterial != null)
        {
            ...
            // Clear any active property block tint so the custom material renders cleanly
            tileRenderer.SetPropertyBlock(null);
            return;
        }

        // 2. Otherwise safely update the color of the renderer material
        Color targetColor = GetColorForType(tileType);

        if (Application.isPlaying)
        {
            // In Play mode, safely update the instantiated material
        ...
        {
            // In Editor / Edit mode, use MaterialPropertyBlock to prevent leaking material assets into the scene
